...
export function EstimatePaper({ project, estimate, companyInfo, miscRate, miscAmount, subtotal, vatAmount, total, categoryOrder, onClose }: Props) {
  ...
  const headerInfo = (
    <div>
      <div className="flex items-end justify-between mb-6 pb-4 border-b-2 border-slate-800">
        <div className="flex items-center gap-3">
          <img src="/logo.png" alt="SOMSSI" className="w-11 h-11 object-contain" />
          <div>
            <h1 className="text-2xl font-black tracking-wider text-slate-800">견 적 서</h1>
            <p className="text-[10px] text-slate-400 tracking-widest mt-0.5">SOMSSI INTERIOR ESTIMATE</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-6 text-xs">
        <div>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">현장 정보</p>
        ...
        <div>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">공급자</p>
          <div className="space-y-1.5">
        ...
        <table className="w-full border-collapse text-[10.5px]">
          <thead>
            <tr style={{ backgroundColor: '#f8fafc' }}>
              <th className="px-2 py-1.5 text-center w-7 text-[10px] font-semibold text-slate-400 border-b-2 border-slate-200">NO</th>
              <th className="px-2 py-1.5 text-left text-[10px] font-semibold text-slate-400 border-b-2 border-slate-200">항목명</th>
              <th className="px-2 py-1.5 text-center w-10 text-[10px] font-semibold text-slate-400 border-b-2 border-slate-200">단위</th>
              <th className="px-2 py-1.5 text-center w-10 text-[10px] font-semibold text-slate-400 border-b-2 border-slate-200">수량</th>
              <th className="px-2 py-1.5 text-right w-16 text-[10px] font-semibold text-slate-400 border-b-2 border-slate-200">단가</th>
              <th className="px-2 py-1.5 text-right w-20 text-[10px] font-semibold text-slate-400 border-b-2 border-slate-200">자재비</th>
              <th className="px-2 py-1.5 text-right w-20 text-[10px] font-semibold text-slate-400 border-b-2 border-slate-200">인건비</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item) => {
              ...
              return (
                <tr key={item.id} className="border-b border-slate-100">
                  <td className="px-2 py-1.5 text-center text-[10px] text-slate-400">{no}</td>
                  <td className="px-2 py-1.5 font-medium text-slate-700 text-[10px]">{item.name || '-'}{item.note && <span className="text-[9px] text-slate-400 ml-1">({item.note})</span>}</td>
                  <td className="px-2 py-1.5 text-center text-slate-500 text-[10px]">{item.unit}</td>
                  <td className="px-2 py-1.5 text-center text-slate-700 font-medium text-[10px]">{item.quantity}</td>
                  <td className="px-2 py-1.5 text-right text-slate-500 text-[10px]">{won(item.unitPrice)}</td>
                  <td className="px-2 py-1.5 text-right font-medium text-[10px]">{won(item.quantity * item.unitPrice)}</td>
                  <td className="px-2 py-1.5 text-right font-medium text-[10px]">{lab > 0 ? won(lab) : '-'}</td>
                </tr>
              );
            })}
          </tbody>
          <tfoot>
            <tr style={{ backgroundColor: '#f8fafc' }}>
              <td colSpan={5} className="px-2 py-1.5 text-right font-bold text-slate-500 text-[10px] border-t-2 border-slate-200">{cat} 소계</td>
              <td className="px-2 py-1.5 text-right font-bold text-[10px] border-t-2 border-slate-200">{won(catMat)}</td>
              <td className="px-2 py-1.5 text-right font-bold text-[10px] border-t-2 border-slate-200">{won(catLab)}</td>
            </tr>
          </tfoot>
        </table>
      </div>
  ...
  const totalsBlock = (
    <div className="flex justify-end">
      <div className="w-64 text-xs">
        {miscRate > 0 && <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-slate-500">공과잡비 ({miscRate}%)</span><span className="font-medium">{won(miscAmount)}원</span></div>}
        <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-slate-500">공급가액</span><span className="font-medium">{won(subtotal)}원</span></div>
        {estimate.includeVat !== false && <div className="flex justify-between py-1.5 border-b border-slate-100"><span className="text-slate-500">부가세 ({estimate.vatRate ?? 10}%)</span><span className="font-medium">{won(vatAmount)}원</span></div>}
        {estimate.discount > 0 && <div className="flex justify-between py-1.5 border-b border-slate-100 text-red-500"><span>할인</span><span className="font-medium">-{won(estimate.discount)}원</span></div>}
        <div className="flex justify-between py-2.5 text-sm font-black border-t-2 border-slate-800 mt-1"><span>총 견적금액</span><span>{won(total)}원</span></div>
      </div>
    </div>
  );
  ...
  const notesBlock = (
    <div>
      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">특이사항</p>
      <div className="rounded-lg px-4 py-3 text-xs text-slate-600 leading-[1.8] whitespace-pre-wrap" style={{ backgroundColor: '#f8fafc' }}>
  ...
  const signBlock = (
    <div className="pt-5 border-t border-slate-200 flex justify-between items-end">
      <p className="text-[10px] text-slate-400">위 금액으로 견적합니다.</p>
      <div className="text-center">
        ...
        <p className="text-[10px] text-slate-400">솜씨인테리어</p>
      </div>
    </div>
  );
      ...
      <div className="paper-stack flex flex-col items-center gap-6 overflow-x-auto">
        {pages.map((idxs, pi) => (
          <div key={pi} className="a4-sheet bg-white text-slate-800 shadow-lg relative"
            ...
            <div className="paper-pageno absolute left-0 right-0 text-center text-[10px] text-slate-400" style={{ bottom: 18 }}>{pi + 1} / {pages.length}</div>
          </div>
        ))}
      </div>
